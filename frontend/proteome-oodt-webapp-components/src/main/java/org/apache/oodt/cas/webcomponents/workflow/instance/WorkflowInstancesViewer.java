...
public WorkflowInstancesViewer(String id, String workflowUrlStr,
    ...
    /* PART OF UNDER-DEVELOPMENT FEATURE: 'LAUNCH WORKFLOW', KEEP DISABLED
     * 
    ArrayList<WorkflowMetadataKeyVal> launchWorkflowMetKeyVals = new ArrayList<WorkflowMetadataKeyVal>();
    launchWorkflowMetKeyVals.add(new WorkflowMetadataKeyVal("key1", "val1"));
    
    add(new ListView("launch_workflow_met_key_vals_list_view", launchWorkflowMetKeyVals) {

		private static final long serialVersionUID = 8993961289390669822L;

		@Override
		protected void populateItem(ListItem item) {
			WorkflowMetadataKeyVal keyValItem = (WorkflowMetadataKeyVal) item.getModelObject();
			
            TextField keyTextField = new TextField("launch_workflow_met_key", new PropertyModel(keyValItem, "key"));
            keyTextField.setOutputMarkupId(true);
            item.add(keyTextField);
          
            TextField valTextField = new TextField("launch_workflow_met_val", new PropertyModel(keyValItem, "val"));
            valTextField.setOutputMarkupId(true);
            item.add(valTextField);            
		}
   ...
   * 
   *
  public class WorkflowMetadataKeyVal {
	  private String key;
	  private String val;
	  
	  public WorkflowMetadataKeyVal(String key, String val) {
		  this.key = key;
		  this.val = val;
	  }
